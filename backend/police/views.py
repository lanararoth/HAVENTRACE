...
class AddPolice(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'message': 'Police officer added successfully!', 'data': serializer.data}, status=status.HTTP_201_CREATED)
...
class UpdatePolice(APIView):
    def put(self, request, pk):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'message': 'Police officer updated successfully!', 'data': serializer.data})
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class DeletePolice(APIView):
    def delete(self, request, pk):
        try:
            police = Police.objects.get(pk=pk)
            police.delete()
            return Response({'message': 'Police officer deleted successfully!'})
        except Police.DoesNotExist:
            return Response({'error': 'Police officer not found!'}, status=status.HTTP_404_NOT_FOUND)