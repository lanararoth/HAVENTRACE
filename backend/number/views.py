...
class AddEmergencyContact(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Emergency contact added successfully."},serializer.data, status=status.HTTP_201_CREATED)
...
class UpdateEmergencyContact(APIView):
    def put(self, request, pk):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Emergency contact updated successfully."},serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class DeleteEmergencyContact(APIView):
    def delete(self, request, pk):
        try:
            ...
            return Response({'message': 'Emergency contact deleted successfully'}, status=status.HTTP_404_NOT_FOUND)
